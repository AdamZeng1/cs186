...
case class PartitionProject(projectList: Seq[Expression], child: SparkPlan) extends UnaryNode {
  ...
  def generateIterator(input: Iterator[Row]): Iterator[Row] = {
    ...
    val relation = DiskHashedRelation(input, keyGenerator)
    val partitions = relation.getIterator()


    new Iterator[Row] {
      var data: Iterator[Row] = Iterator.empty

      def hasNext() = {
        // IMPLEMENT ME
        data.hasNext || fetchNextPartition()
      }
      ...
      private def fetchNextPartition(): Boolean  = {
        ...
        if (partitions.hasNext) {
          data = cacheGenerator(partitions.next().getData())
          true
        } else {
          relation.closeAllPartitions()
          false
        }
      }
    }
