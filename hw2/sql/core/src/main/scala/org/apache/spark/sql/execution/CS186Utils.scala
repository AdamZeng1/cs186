    ...
    { input =>
      new Iterator[Row] {
        ...
        def next() = {
          // IMPLEMENT ME
          val row = input.next()
          val result = (preUdfProjection(row) ++ cachedProject(row) ++ postUdfProjection(row)) .toArray
          new GenericRow(result)
        }

        private[this] def cachedProject(row: Row): Row = {
          val key = cacheKeyProjection(row)
          if (!cache.containsKey(key)) cache.put(key, udfProject(row))
          cache.get(key)
        }
      }
    }
