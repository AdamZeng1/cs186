...
private[sql] class DiskPartition (
  ...
  def insert(row: Row) = {
    // IMPLEMENT ME
    if (inputClosed) throw new SparkException("Should not be writing to partition after the input had been closed.")
    data.add(row)
  ...
  private[this] def measurePartitionSize(): Int = {
    CS186Utils.getBytesFromList(data).length
  }
  ...
  def getData(): Iterator[Row] = {
    ...
    new Iterator[Row] {
      ...
      override def hasNext() = {
        // IMPLEMENT ME
        currentIterator.hasNext || chunkSizeIterator.hasNext && fetchNextChunk()
      }
      ...
      private[this] def fetchNextChunk(): Boolean = {
        ...
        if (chunkSizeIterator.hasNext) {
          byteArray = new Array(chunkSizeIterator.next())
  ...
  def closeInput() = {
    // IMPLEMENT ME
    if (!writtenToDisk && !data.isEmpty) spillPartitionToDisk()
    outStream.close()
    inputClosed = true
  }
...
private[sql] object DiskHashedRelation {
  ...
  def apply (
    ...
    for(i <- partitions.indices) {
      partitions(i) = new DiskPartition(s"relation-partition-$i", blockSize)
    }

    input.foreach(row => {
      val key = keyGenerator.apply(row)
      val bucket = key.hashCode() % size
      partitions(bucket).insert(row)
    })
