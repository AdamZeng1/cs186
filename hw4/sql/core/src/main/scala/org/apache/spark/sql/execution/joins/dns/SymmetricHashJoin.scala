...
import org.apache.spark.sql.catalyst.expressions.{Expression, InterpretedProjection, JoinedRow, Projection}
...
trait SymmetricHashJoin {
  ...
  protected def symmetricHashJoin(leftIter: Iterator[Row], rightIter: Iterator[Row]): Iterator[Row] = {
    new Iterator[Row] {
      ...
      // IMPLEMENT ME
      type HashedRelation = mutable.HashMap[Row, mutable.MutableList[Row]]
      val rows: mutable.Queue[Row] = new mutable.Queue[Row]()
      val inner = new HashedRelation()
      val outer = new HashedRelation()
      ...
      def findNextMatch(): Boolean = {
        // IMPLEMENT ME
        while(leftIter.hasNext && isInner || rightIter.hasNext && !isInner) {
          val tuple = nextTuple()
          val key = projection(tuple)
          putTo(key, tuple, innerTable)
          if (outerTable.contains(key)) {
            outerTable(key).foreach(row => {
              rows.enqueue(new JoinedRow(tuple, row))
            })
            return true
          }
          switchRelations()
      ...
      private def innerTable: HashedRelation = {
        if (isInner) inner else outer
      }

      private def outerTable: HashedRelation = {
        if (isInner) outer else inner
      }

      private def nextTuple(): Row = {
        if (isInner) leftIter.next() else rightIter.next()
      }

      private def projection: Projection = {
        if (isInner) leftKeyGenerator else rightKeyGenerator
      }

      private def putTo(key: Row, value: Row, table: HashedRelation): Unit = {
        if (table.contains(key)) {
          table(key) += value
        } else {
          table.put(key, new mutable.MutableList[Row]() += value)
        }
      }
    }
  }
