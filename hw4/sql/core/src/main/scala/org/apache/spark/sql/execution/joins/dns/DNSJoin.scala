...
trait DNSJoin {
  ...
  def hashJoin(input: Iterator[Row]): Iterator[Row] = {
    new Iterator[Row] {
      // IMPLEMENT ME
      val dnsCache: scala.collection.mutable.Map[String, Row] = new scala.collection.mutable.HashMap()
      val joinedTuples: mutable.Queue[Row] = new mutable.Queue()
      val requestedTuples: ConcurrentHashMap[Int, Row] = new ConcurrentHashMap()
      val respondedTuples: ConcurrentHashMap[Int, Row] = new ConcurrentHashMap()
      var id: Int = 0
      ...
      override def hasNext() = {
        // IMPLEMENT ME
        joinedTuples.nonEmpty || !requestedTuples.isEmpty || !respondedTuples.isEmpty
      }
      ...
      private def makeRequest() = {
        ...
        val ip = rowToIpString(tuple)

        requestedTuples.put(id, tuple)

        if (dnsCache.contains(ip)) {
          TupleJoiner.accept(id, dnsCache(ip))
        } else {
          DNSLookup.lookup(id, ip, respondedTuples, requestedTuples)
        }
      ...
      /**
        *  This method makes initial async calls to DNS lookup.
        *  The amount of calls is bounded by the lesser of the size of the input or requestBufferSize constant.
        */
      private def initialize(): Unit = {
        while (input.hasNext && id < requestBufferSize) makeRequest()
      }

      private def rowToIpString(row: Row): String = {
        leftKeyGenerator(row).getAs[String](0) // stinks...
      }

      private def processResponses(): Unit = {
        while(joinedTuples.isEmpty) {
          respondedTuples forEach TupleJoiner
        }
      }

      private object TupleJoiner extends BiConsumer[Int, Row] {
        override def accept(id: Int, response: Row): Unit = {
          val tuple = respondedTuples.remove(id)
          val ip = rowToIpString(tuple)
          joinedTuples.enqueue(new JoinedRow(tuple, response))
          dnsCache.put(ip, response)
          if (input.hasNext) {
            makeRequest()
          }
        }
