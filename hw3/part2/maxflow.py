...
conn = psycopg2.connect(database="part2", user="ivolnov")
    ...
    while True:
        ...
        while not sink_path_found:
            ...
            db.execute("""
                    ...
                    INTO tmp
                    FROM paths AS p
                    JOIN edge AS e ON p.nodes[array_length(p.nodes, 1)] = e.src
                    WHERE NOT p.nodes @> array[e.dst] AND NOT e.capacity = 0;
        ...
        db.execute("""
            ...
            constraining_capacity(capacity) AS (
                SELECT MIN(e.capacity) FROM path_edges AS p, edge AS e WHERE e.id = p.path_edge
            )
        ...
        db.execute("""
            WITH updates(id, new_capacity) AS (
                SELECT e.id, e.capacity - f.flow FROM edge AS e, flow_to_route AS f WHERE e.id = f.edge_id
                UNION
                SELECT e.id, e.capacity + f.flow FROM edge AS e, flow_to_route AS f, flip_edge AS flip WHERE e.id = flip.reverse_id AND f.edge_id = flip.forward_id
            )
            UPDATE edge
