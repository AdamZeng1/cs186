...
def get_all_transaction_amounts():
    ...
    """
    # Execute database query
    db.execute("SELECT transaction_amt, state FROM intercommittee_transactions WHERE transaction_amt > 0;")

    # Package into output
    return [{'amount' : tuple[0] , 'state' : tuple[1]} for tuple in db.fetchall()]
...
def get_total_transaction_amounts_by_state():
    ...
    # Execute database query
    db.execute("SELECT state, SUM(transaction_amt) FROM intercommittee_transactions WHERE transaction_amt > 0 GROUP BY state HAVING state IS NOT NULL;")

    # Package into list of dictionaries
    return [{'state' : tuple[0], 'total_amount' : tuple[1]} for tuple in db.fetchall()]
