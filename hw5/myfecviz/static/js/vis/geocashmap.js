...
USCashMap.prototype.render = function (data) {
    ...
    that.moneyColorScale = d3.scale.sqrt()
        .domain([
    ...
    this.states.transition().duration(200)
        .attr("fill", function(d) {
            ...
            return that.moneyColorScale(d['total_amount']);  // Return a hexcode
        });
    ...
    this.states.on('mouseover', function(d, e, p){
        ...
        var stateCode = d['state'],
            stateName = that.stateNameMap.get(stateCode),
            amount = d['total_amount'];

        that.addStateToSelection(stateCode);

        that.states
            .filter(function (s) {
                return s['state'] == stateCode;
            })
            .attr('fill', '#4CAF50');

        that.setInspectionInfo(stateName, amount);
    });
    ...
    this.states.on('mouseout', function(d, e, p){
        ...
        that.states
            .filter(function (s) {
                return s['state'] == d['state'];
            })
            .attr('fill', function (s) {
                return that.moneyColorScale(s['total_amount']);
            });
    });
